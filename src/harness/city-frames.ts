...
export const CITY_TUNING = makeTuning({ SETTLEMENTS_ENABLED: 1, TERRAIN_ROUGH_FRACTION: 0.12 });
...
export function renderCity(view: CityView, options: CitySceneOptions, width: number, height: number, focus = true): Frame {
  const b = sceneBounds(view.tiles);
...
function main(): void {
  ...
  console.log(`wrote ${CITY_GOLDEN.name}, city-preview.png and city-whole.png to ${FRAMES_DIR}`);
}
